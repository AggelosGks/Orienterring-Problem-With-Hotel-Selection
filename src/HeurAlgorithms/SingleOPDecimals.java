...
public class SingleOPDecimals {
	...
	/**
	 *Constructor for the single OP problem between two hotels.
	 *We do not collect points of interest we consider the whole graph.
	 *This method was used for our experimental results in the single OP problem 
	 *on the instances published by Tsiligrides.
	 * @param start
	 * @param finish
	 * @param Td
	 * @param inline_part
	 */
	public SingleOPDecimals(Hotel start, Hotel finish,double Td){
		this.start=start;
		this.end=finish;
		this.Td=Td;
		this.checker=Td+0.1;
		...
		//System.out.println("Starting size: "+candidates.size());
		//System.out.println("List: "+Computation.ListToString(candidates));
		this.step_size=0.1;
		Preprocess();
	...
	/**
	 * Constructor for partial sequential technique when we have to remove points of 
	 * interest that have already been visited.
	 * @param start the start hotel of the trip
	 * @param finish the finish hotel of the trip
	 * @param Td the time budget
	 * @param inline_part map contains pois to be excluded
	 */
	public SingleOPDecimals(Hotel start, Hotel finish,double Td,TreeMap<Integer,Boolean> inline_part){
		this.start=start;
		this.end=finish;
		this.Td=Td;
		this.checker=Td+0.1;
		this.start_visitors=new TreeMap<Double,ArrayList<POI>>();
		System.out.println(" ");
		System.out.println("Start Hotel: "+start.node_id+" Finish Hotel: "+end.node_id+" time budget: "+Td+"InLine");
	...
	/**
	 * Initializes the data structures for each point of interest that 
	...
	public void eXecute(boolean print){
		...
		while(t<=checker&&control){//viewpoint at value t
			...
			while(!stack.isEmpty()){//until O(n) solutions have been created
				...
				//sort them according the time allowed to spent.
				max_score=0;
				max_vis=null;
				opt_seq.clear();
				opt_seq=null;
